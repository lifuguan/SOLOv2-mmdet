...
class SOLOv2Head(nn.Module):
    ...
    def solov2_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, num_grid \
            ...
            # mass center
            gt_masks_pt = torch.from_numpy(gt_masks.masks).to(device=device)
    ...
    def get_seg(self, cate_preds, kernel_preds, seg_pred, img_metas, cfg, rescale=None):
        ...
        for img_id in range(len(img_metas)):
            # based on solo_head.py
            res_ins = InstanceData(img_metas[img_id])
            cate_pred_list = [
            ...
            if result is not None:
                res_ins.masks = result[0]
                res_ins.labels = result[1]
                res_ins.scores = result[2]
            result_list.append(res_ins)
        return result_list
